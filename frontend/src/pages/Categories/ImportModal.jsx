...
export default function ImportModal({ onClose }) {
  ...
  const downloadTemplate = async () => {
    try {
      const blob = await api.get('/categories/template', { responseType: 'blob' })
  ...
  const runPreview = useCallback(async (selectedFile) => {
    ...
    try {
      ...
      const data = await api.post('/categories/import?dry_run=true', fd, {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const fd = new FormData()
      fd.append('file', file)
      const data = await api.post('/categories/import?dry_run=false', fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      setResult(data)
      setStep(3)
      if (data.summary.inserted > 0) qc.invalidateQueries({ queryKey: ['categories-admin'] })
    } catch (e) {
      ...
      <div className="w-full max-w-4xl max-h-[90vh] flex flex-col rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[15px] font-bold" style={{ color: 'var(--text)' }}>Import Categories from Excel</h2>
              ...
              <div className="rounded-xl p-4 flex items-center gap-4"
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-muted)' }}>
                    Includes 6 columns for defining categories and subcategories
                  </p>
                </div>
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr style={{ backgroundColor: 'var(--surface-2)', borderBottom: '1px solid var(--border)' }}>
                      {['Row', 'Type', 'Name (EN)', 'Parent Category', 'Status', 'Reason'].map(h => (
                  ...
                  <tbody>
                    {preview.results.map((r, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid var(--border)', backgroundColor: i % 2 === 0 ? 'var(--surface)' : 'var(--surface-2)' }}>
                        ...
                        <td className="px-3 py-2" style={{ color: 'var(--text)' }}>{r.type}</td>
                        <td className="px-3 py-2 font-semibold" style={{ color: 'var(--text)' }}>{r.name_en}</td>
                        <td className="px-3 py-2" style={{ color: 'var(--text-muted)' }}>{r.parent || '—'}</td>
                ...
                <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                  ...
                  {result.results.filter(r => r.status === 'error').map((r, i) => (
                    <div key={i} className="flex items-start gap-3 px-4 py-3"
                      ...
                      <span className="font-semibold text-[12px]" style={{ color: 'var(--text)' }}>{r.type} {r.name_en}</span>
        ...
        <div className="flex items-center justify-between px-6 py-4 shrink-0"
          ...
          {step === 2 && (
            <button onClick={handleConfirm} disabled={readyCount === 0 || loading}
              ...
              Confirm — Import {readyCount} Row{readyCount !== 1 ? 's' : ''}
            </button>
          )}
          {step === 3 && (
